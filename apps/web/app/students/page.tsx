...
const studentListOptions = (page: number) =>
  queryOptions({
    queryKey: ["students", { page }],
    queryFn: () => getStudentPage(page),
  });

const studentTotalPagesOptions = () =>
  queryOptions({
...
function StudentPageContent() {
  ...
  } = useQuery(studentListOptions(page));
    ...
    <div className="w-4/5 m-2 ml-0">
      ...
      <div className="flex flex-col my-8 p-2 bg-(--dark-white) rounded-xl">
        ...
        <div className="bg-white no-scrollbar flex flex-col rounded-xl mt-2">
          {students.map((student: studentCreateDto, index: number) => (
            <div
              key={student.id}
              className={`flex flex-row py-3 border-black/10  ${index < students.length - 1 ? "border-b" : "border-none"}`}
            >
