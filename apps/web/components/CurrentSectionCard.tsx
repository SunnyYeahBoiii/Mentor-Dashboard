...
export default function CurrentSectionCard({
  id,
  name,
  className,
  startTime,
    ...
    <div className="w-full rounded-xl bg-(--dark-white) flex flex-col justify-between p-2 py-2 gap-2">
      <span className="flex flex-row justify-between gap-5">
        ...
        <div className="flex flex-row flex-1 gap-5">
          <Link
            className="flex flex-1 px-2 py-1 rounded-sm justify-center items-center bg-white hover:bg-green-300 cursor-pointer"
            href={`/current-sessions/end-session/${id}`}
          >
            <p className="text-center">Kết thúc</p>
          </Link>
          <div
            className="flex flex-1 rounded-sm px-2 py-1 justify-center items-center bg-white hover:bg-red-300 cursor-pointer"
        ...
        <div className="flex-3 bg-white rounded-sm">
          <div className="mx-2 my-1">
            ...
            <p className="text-left">
              ...
              {startTime.toLocaleTimeString()} - Đang diễn ra
            </p>
          </div>
        </div>
      ...
      <span className="flex flex-row justify-between gap-5">
        <div className="flex-10 bg-white rounded-sm px-2 py-1">
          ...
          <span className="flex flex-row gap-4 items-center">
            <a
              className="text-left text-blue-500 hover:text-blue-800 cursor-pointer"
              href={meetingLink}
            >
              {meetingLink}
            </a>
